...
\IEEEPARstart{R}{obot}  has been widely used in industry and research. In Robotics, inverse kinematics is a basic problem which has been 
...
Robot manipulator is multiple joints system.  To calculate the position of the end effector in the world frame, local frame system for joints should be created and each joint has one aligned local frame.
 Let $\Tb_0$ be the world/base frame and $\Tb_i^{i+1}$ be the transformation matrix from the $i$th frame to $i+1$th frame.  Then we have 
\[
...
Inverse kinematics usually comes with  specific robot tasks.  So when we try to solve this problem, it is necessary to consider the task constraints. Moreover, robot has it's own workspace, which also can cause limitations for the inverse kinematics problem. \\ 
...
\begin{itemize}
\item All the joint variables should be within the limit of the robot joints
